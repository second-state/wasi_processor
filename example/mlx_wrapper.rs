...
#[cfg(not(target_family = "wasm"))]
use mlx_sys::{mlx_array, mlx_default_cpu_stream_new, mlx_load};
...
impl ArrayExt for Array {
    ...
    fn to_vec<T: TryFrom<i32> + Copy>(&self) -> Result<Vec<T>, Box<dyn std::error::Error>> {
        ...
        println!(
            "Array shape: [{:#?}], total elements: {}",
            shape_info, total_elements
        );
        ...
        if dtype == Dtype::Int32 {
            ...
            for &x in result.unwrap().as_slice::<i32>() {
                match T::try_from(x) {
                    ...
                    Err(_) => {
                        return Err(
                            format!("Element {} cannot be converted to target type", x).into()
                        )
                    }
                }
            }
