...
impl Gemma3ImageProcessor {
    ...
    pub fn from_pretrained(model_dir: &str) -> Self {
        ...
        let proc_str =
            fs::read_to_string(&proc_path).expect("Unable to read processor_config.json");
        let proc_json: serde_json::Value =
            serde_json::from_str(&proc_str).expect("Failed to parse processor_config.json");
