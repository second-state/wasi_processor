...
impl Gemma3Processor {
    ...
    pub fn process(&self, text: Option<&str>, image: Option<&[u8]>) -> BatchFeature {
        match (text, image) {
            (Some(text), Some(img)) => self.process_text_and_image(text, img),
            (Some(text), None) => self.process_text_only(text),
            (None, Some(img)) => self.process_image_only(img),
            _ => {
    ...
    pub fn process_batch(&self, texts: Vec<&str>, images: Option<Vec<&[u8]>>) -> BatchFeature {
            ...
            Some(imgs) if imgs.len() == batch_size => {
                ...
                for (text, img) in texts.iter().zip(imgs.iter()) {
                    ...
                    self.collect_batch_results(
                        &result,
                        &mut all_input_ids,
                        &mut all_attention_masks,
                        &mut all_token_type_ids,
                        &mut all_pixel_values,
                        &mut all_num_crops,
                    );
                }
            }
        ...
        BatchFeature {
            ...
            pixel_values: if all_pixel_values.is_empty() {
                None
            } else {
                Some(all_pixel_values)
            },
            num_crops: if all_num_crops.is_empty() {
                None
            } else {
                Some(all_num_crops)
            },
