...
impl Gemma3Tokenizer {
    ...
    pub fn from_pretrained(model_dir: &str) -> Self {
        ...
        let config_str =
            fs::read_to_string(&config_path).expect("Failed to read tokenizer_config.json");
        let config: TokenizerConfig =
            serde_json::from_str(&config_str).expect("Failed to parse tokenizer_config.json");

        // Read image_token_id from config.json
        let model_config_path = format!("{}/config.json", model_dir);
        let model_config_str =
            fs::read_to_string(&model_config_path).expect("Failed to read config.json");
        let model_config: serde_json::Value =
            serde_json::from_str(&model_config_str).expect("Failed to parse config.json");
    ...
    pub fn get_vocab(&self) -> HashMap<String, usize> {
        ...
        vocab
            .into_iter()
            .map(|(token, id)| (token, id as usize))
            .collect()
    }
