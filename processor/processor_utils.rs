...
pub fn prepare_inputs(
    ...
    for image_path in image_paths {
        let image_bytes = fs::read(image_path).expect(&format!("Cannot read image: {}", image_path));
...
fn process_text_only(
    ...
    if !batch_feature.input_ids.is_empty() {
        ...
        model_inputs.insert("input_ids".to_string(), NDTensor::new(data, shape));
    }
    ...
    if !batch_feature.attention_mask.is_empty() {
        ...
        let shape = vec![1, attention_mask.len()];
        let data: Vec<f32> = attention_mask.iter().map(|&mask| mask as f32).collect();
        model_inputs.insert("mask".to_string(), NDTensor::new(data, shape));
    }
    ...
    if !batch_feature.token_type_ids.is_empty() {
        ...
        let shape = vec![1, token_type_ids.len()];
        let data: Vec<f32> = token_type_ids.iter().map(|&id| id as f32).collect();
        model_inputs.insert("token_type_ids".to_string(), NDTensor::new(data, shape));
    }
...
fn process_images_and_text(
    ...
    if !all_pixel_values.is_empty() {
        ...
        let channels = 3;
        let num_images = images_bytes.len();

        // Multi-image shape: [num_images, channels, height, width]
        let shape = vec![num_images, channels, height as usize, width as usize];
        model_inputs.insert(
            "pixel_values".to_string(),
            NDTensor::new(all_pixel_values, shape),
        );
    }

    // Add input_ids (2D tensor [1, seq_len])
    if !final_input_ids.is_empty() {
        let shape = vec![1, final_input_ids.len()];
    ...
    if !final_attention_mask.is_empty() {
        let shape = vec![1, final_attention_mask.len()];
        let data: Vec<f32> = final_attention_mask.iter().map(|&mask| mask as f32).collect();
        model_inputs.insert("mask".to_string(), NDTensor::new(data, shape));
    }

    // Add token_type_ids (2D tensor [1, seq_len])
    if !final_token_type_ids.is_empty() {
        let shape = vec![1, final_token_type_ids.len()];
    ...
    if !all_num_crops.is_empty() {
        let shape = vec![all_num_crops.len()];
        let data: Vec<f32> = all_num_crops.iter().map(|&crop| crop as f32).collect();
        model_inputs.insert("num_crops".to_string(), NDTensor::new(data, shape));
    }
